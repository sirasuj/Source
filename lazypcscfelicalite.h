...
namespace LazyPCSCFelicaLite
{
	...
	private:
		...
		void debugInfo2(const char *FUNCSIGname, const char *fmt, ...)
		{
			if (debugFlag == false)
			{
				return;
			}
			printf("[%s.%s]", InstanceName.c_str(), FUNCSIGname);
			va_list va;
			va_start(va, fmt);
			vprintf(fmt, va);
			va_end(va);
	...
	public:
		...
		{
			debugFlag = debug;
			debugInfo2(__FUNCSIG__, " PCSCFelicaLite(bool debug)\n");
		}
		...
		PCSCFelicaLite(bool debug, string name)
		{
			debugFlag = debug;
			InstanceName = name;
			debugInfo2(__FUNCSIG__, " PCSCFelicaLite(bool debug,string name)\n");
		}
		...
		{
			debugInfo2(__FUNCSIG__, "カードへの自動接続開始\n");
			...
			if (!isCardSet())
			{
				debugInfo2(__FUNCSIG__, "カードが見つからない\n");
				return false;
			}
			...
			{
				debugInfo2(__FUNCSIG__, "Felicaではないカードを検出した\n");
				return false;
			}
			disconnectCard();
			...
			debugInfo2(__FUNCSIG__, "カードへの接続に成功\n");
			return true;
		}
		...
		void openService()
		{
			...
			{
				hContext = NULL;
				debugInfo2(__FUNCSIG__, " スマートカードリソースマネージャへの接続失敗:%X\n", res);
				throw PCSCFatalException("スマートカードリソースマネージャへの接続に失敗", PCSCFatalException::FAILD_TO_ESTABLISH, res);
			}
			debugInfo2(__FUNCSIG__, " スマートカードリソースマネージャへの接続に成功\n");
			...
			if (debugFlag)
			{
				debugInfo2(__FUNCSIG__, " カードリーダー名リスト\n");
				int i = 0;
				for (string name : ReaderNameList)
				{
					debugInfo2(__FUNCSIG__, "[%d] %s\n", i, name.c_str());
					i++;
				}
			}
			...
			if (activeReaderName == "")
			{
				debugInfo2(__FUNCSIG__, " 利用できるカードリーダ名が見つかりませんでした\n");
		...
		void closeService()
		{
			...
			if (hContext != NULL)
			{
				...
				debugInfo2(__FUNCSIG__, " スマートカードリソースマネージャを解放\n");
			}
		...
		bool isCardSet(uint32_t state = SCARD_STATE_UNAWARE, uint32_t timeout = 0)
		{
			...
			if (timeout == INFINITE)
				debugInfo2(__FUNCSIG__, " 状態の取得を開始(timeout=無限に待機)\n");
			else if (timeout == 0)
				;
			else
				debugInfo2(__FUNCSIG__, " 状態の取得を開始(timeout=%dms)\n", timeout);

			LONG res = SCardGetStatusChangeA(hContext, (DWORD)timeout, &readerState, 1);
			if (res == SCARD_E_TIMEOUT)
			{
				debugInfo2(__FUNCSIG__, " 読み取りタイムアウト\n");
				return false;
			}
			...
			{
				debugInfo2(__FUNCSIG__, " カードリーダーが接続されていないか利用不能\n");
			...
			{
			debugInfo2(__FUNCSIG__, " システムコードスキャンを試行\n");
			try
			{
			...
			{
			//失敗したならばカードがない
			debugInfo2(__FUNCSIG__, " カードが取り外された\n");
			Sleep(2000);//高速
			return false;
			}
			}
			*/
			debugInfo2(__FUNCSIG__, " カードがセットされています %X\n", readerState.dwEventState);
			return true;
		}
		...
		bool waitForSetCard(uint32_t timeout = INFINITE)
		{
			...
			debugInfo2(__FUNCSIG__, " カードがセットされるまで待ちます\n");
			//状態変化するまで永遠に待機
		...
		void connectCard(bool exclusive = true)
		{
			...
			debugInfo2(__FUNCSIG__, " カードに接続しました(カード種別:%02X, システムコード:%04X)\n", cardTypecode, cardSystemcode);
		}
		...
		void connectDirect()
		{
			...
			if (res != SCARD_S_SUCCESS)
			{
				...
				if (res == SCARD_W_REMOVED_CARD)
				{
					debugInfo2(__FUNCSIG__, " 直接接続に失敗\n");
		...
		void disconnectCard()
		{
			if (hCard != NULL)
			{
				debugInfo2(__FUNCSIG__, " 接続を切断しました\n");
		...
		void readBinary(uint16_t adr, uint8_t dat[16])
		{
			...
			if (res == SCARD_W_REMOVED_CARD)
			{
				debugInfo2(__FUNCSIG__, " カードが取り外されました．\n");
			...
			if (res != SCARD_S_SUCCESS)
			{
				if (res == SCARD_W_REMOVED_CARD)
				{
					debugInfo2(__FUNCSIG__, " PC/SCコマンド発行中にカードが取り外されました．\n");
					throw PCSCCardRemovedException("PC/SCコマンド発行中にカードが取り外されました．");
				}
				if (res == 0x458)
				{
					debugInfo2(__FUNCSIG__, " カードハンドルが無効になりました．\n");
					throw PCSCCardRemovedException("カードハンドルが無効になりました．");
				}
				if (res == ERROR_BAD_COMMAND)
				{
					debugInfo2(__FUNCSIG__, " コマンドに誤りがあるか不適切な状態です (直接接続していませんか？)%X\n", res);
				}
				else
				{
					debugInfo2(__FUNCSIG__, " コマンドに誤りがあるか不適切な状態です %X\n", res);
				}
			...
			{
				debugInfo2(__FUNCSIG__, " アドレス:%04X カードはエラー応答しました．SW1=%02X,SW2=%02X\n", adr, SW1, SW2);
				...
				{
					debugInfo2(__FUNCSIG__, " Felicaカード側処理エラーです．コード:%02X,%02X\n", pbRecvBuffer[0], pbRecvBuffer[1]);
			...
			{
				debugInfo2(__FUNCSIG__, " 応答サイズ異常 %d\n", pcbRecvLength);
			...
			if (debugFlag)
			{
				debugInfo2(__FUNCSIG__, " アドレス:%04X 読み込み完了\n", adr);
		...
		void updateBinary(uint16_t adr, uint8_t dat[16])
		{
			...
			if (SW1 != 0x90 || SW2 != 0x00)
			{
				...
				if (SW1 == 0x91 && SW2 == 0x00)
				{
					...
					{
						debugInfo2(__FUNCSIG__, " Felicaカード側処理警告です(メモリ寿命近し)コード:%02X,%02X\n", pbRecvBuffer[0], pbRecvBuffer[1]);
					}
					else{
						debugInfo2(__FUNCSIG__, " Felicaカード側処理エラーです．コード:%02X,%02X\n", pbRecvBuffer[0], pbRecvBuffer[1]);
						debugInfo2(__FUNCSIG__, " %s\n", FellicaErrorInfo(pbRecvBuffer[1]));
			...
			if (debugFlag)
			{
				debugInfo2(__FUNCSIG__, "  アドレス:%04X 書き込み成功\n", adr);
				debugInfo2(__FUNCSIG__, " 内容:");
				for (DWORD i = 0; i < 16; i++)
				{
					printf("%02X,", dat[i]);
		...
		void getPaSoRiSerialNumber(char *serialNumberString = NULL)
		{
			...
			debugInfo2(__FUNCSIG__, " カードリーダーシリアルナンバー:%s\n", lpOutBuffer);
		}
		...
		uint32_t readUID(uint8_t UID[] = NULL)
		{
			...
			if (SW1 != 0x90 || SW2 != 0x00)
			{
				...
				debugInfo2(__FUNCSIG__, " カードはエラー応答しました．SW1=%02X,SW2=%02X\n", SW1, SW2);
				throw PCSCErrorException("PC/SC カードエラー応答", SW1, SW2);
			}
		...
		uint8_t readCardTypeCode()
		{
			...
			if (debugFlag)
			{
				debugInfo2(__FUNCSIG__, " カード種別:%02X\n", pbRecvBuffer[0]);
				switch (pbRecvBuffer[0])
				{
				case CARD_TYPE_FELICA: debugInfo2(__FUNCSIG__, " このカードはFelicaです．\n"); break;
				case CARD_TYPE_ISO14443A: debugInfo2(__FUNCSIG__, " このカードはISO14443Aです．\n"); break;
				case CARD_TYPE_ISO14443B: debugInfo2(__FUNCSIG__, " このカードはISO14443Bです．\n"); break;
				case CARD_TYPE_PICOPASSB: debugInfo2(__FUNCSIG__, " このカードはPICOPASSBです．\n"); break;
				case CARD_TYPE_NFC_TYPE_1: debugInfo2(__FUNCSIG__, " このカードはNFC_TYPE_1です．\n"); break;
				case CARD_TYPE_MIFARE_EC: debugInfo2(__FUNCSIG__, " このカードはMIFARE_ECです．\n"); break;
				case CARD_TYPE_ISO14443A_4A: debugInfo2(__FUNCSIG__, " このカードはISO14443A_4Aです．\n"); break;
				case CARD_TYPE_ISO14443B_4B: debugInfo2(__FUNCSIG__, " このカードはISO14443B_4Bです．\n"); break;
				case CARD_TYPE_TYPE_A_NFC_DEP: debugInfo2(__FUNCSIG__, " このカードはTYPE_A_NFC_DEPです．\n"); break;
				case CARD_TYPE_FELICA_NFC_DEP: debugInfo2(__FUNCSIG__, " このカードはFELICA_NFC_DEPです．\n"); break;
				default: debugInfo2(__FUNCSIG__, " このカードの種類は不明です．\n"); break;
				}
			}
		...
		bool isFelicaLite()
		{
			if (!isFelica())
			{
				debugInfo2(__FUNCSIG__, " このカードはFelicaではない\n");
				return false;
			}

			if ((getSystemcode() != SYSTEMCODE_NFC_TYPE3) && (getSystemcode() != SYSTEMCODE_FELICALITE))
			{
				debugInfo2(__FUNCSIG__, " このカードはFelica Liteではない\n");
				return false;
			}
			debugInfo2(__FUNCSIG__, " このカードはFelica Lite系です\n");
			return true;
		}
		...
		{
		debugInfo2(__FUNCSIG__," このカードはFelicaではない(カード種別が違う)\n");
		return false;
		}
		...
		if(size != 8)
		{
		debugInfo2(__FUNCSIG__," このカードはFelicaではない(PMmが8byteではない)\n");
		return false;
		}

		if(PMm[1] < 0xF0)
		{
		debugInfo2(__FUNCSIG__," このカードはFelica Liteではない(StandardおよびモバイルFelicaなど)\n");
		return false;
		}

		switch( PMm[1] )
		{
		case 0xF0: debugInfo2(__FUNCSIG__," このカードはFelica Liteです．(RC-S965)\n");break;
		case 0xF1: debugInfo2(__FUNCSIG__," このカードはFelica Lite-Sです．(RC-S966)\n"); break;
		case 0xF2: debugInfo2(__FUNCSIG__," このカードはFelica Linkです．(RC-S967/Lite-S/Lite-S HT)\n"); break;
		case 0xE0: debugInfo2(__FUNCSIG__," このカードはFelica Plugです．(RC-S926)\n"); break;
		case 0xE1: debugInfo2(__FUNCSIG__," このカードはFelica Linkです．(RC-S967/Plug)\n"); break;
		case 0xFF: debugInfo2(__FUNCSIG__," このカードはNFC仕様のFelicaです．\n"); break;
		default :  debugInfo2(__FUNCSIG__," このカードはFelica Liteっぽいです．(不明)\n"); break;
		}
		return true;
		}
		...
		if(PMm[1] == 0xF1 )
		{
		debugInfo2(__FUNCSIG__," このカードはFelica Lite-Sです．(RC-S966)\n");
		return true;
		}else{
		debugInfo2(__FUNCSIG__," このカードはFelica LiteですがLite-Sではありません．\n");
		return false;
		}
		}
		*/
		void readCardTypeString(char *cardTypeString = NULL)
		{
			...
			if (debugFlag)
			{
				debugInfo2(__FUNCSIG__, " カード種別名称:");
				for (DWORD i = 0; i < pcbRecvLength - 2; i++) //最後の2バイトはレスポンス
				{
		...
		uint16_t readSystemcode(uint16_t scancode = SYSTEMCODE_ANY)
		{
			...
			if (SW1 != 0x90 || SW2 != 0x00)
			{
				if (SW1 == 0x63 && SW2 == 0x00)
				{
					...
					debugInfo2(__FUNCSIG__, " 該当するシステムコードのカードは見つかりませんでした．(タイムアウト)\n");
			...
			{
				debugInfo2(__FUNCSIG__, " 期待される応答ではない．長さ%d (本来:19) 応答:%02X\n", pcbRecvLength - 2, pbRecvBuffer[0]);
			...
			debugInfo2(__FUNCSIG__, " システムコード:%04X\n", systemcode);
		...
		bool isNdefEnabled()
		{
			...
			if (mc[3] == 0x01)
			{
				debugInfo2(__FUNCSIG__, " このカードはNDEFが有効です\n");
				return true;
			}
			else
			{
				debugInfo2(__FUNCSIG__, " このカードはNDEFが無効です\n");
				return false;
			}
		}
		...
		bool isFirstIssued()
		{
			...
			if (mc[2] != 0xFF)
			{
				debugInfo2(__FUNCSIG__, " このカードは1次発行済みです\n");
				return true;
			}
			else
			{
				debugInfo2(__FUNCSIG__, " このカードはまだ1次発行されていません\n");
				return false;
			}
		}
		...
		bool isSecondIssued()
		{
			...
			if ((mc[1] & 0x80) == 0)
			{
				debugInfo2(__FUNCSIG__, " このカードは2次発行済みです\n");
				return true;
			}
			else
			{
				debugInfo2(__FUNCSIG__, " このカードはまだ2次発行されていません\n");
				return false;
			}
		}
		...
		void FirstIssue()
		{
			...
			debugInfo2(__FUNCSIG__, " 1次発行を行いました\n");
		}
		...
		void enableNDEF(bool en = true)
		{
			...
			if (en)
			{
				debugInfo2(__FUNCSIG__, " NDEF有効化\n");
				mc[3] = 0x01;
			}
			else
			{
				debugInfo2(__FUNCSIG__, " NDEF無効化\n");
				mc[3] = 0x00;
			}
			updateBinary(ADDRESS_MC, mc);
		}
		...
		void writeNdefURI(uint8_t mode, const char str[])
		{
			...
			debugInfo2(__FUNCSIG__, " NDEF書き込み完了 %s\n", str);
		}
		...
		void makeSessionKey(uint8_t RC1[8], uint8_t RC2[8], uint8_t CK1[8], uint8_t CK2[8], uint8_t SK1[8], uint8_t SK2[8])
		{
			...
			debugInfo2(__FUNCSIG__, " セッションキーを生成しました\n");
			if (debugFlag)
			{
		...
		void makeReadMAC_A(uint16_t BlockAdr, uint8_t BLOCK[16], uint8_t RC1[8], uint8_t SK1[8], uint8_t SK2[8], uint8_t MAC[8])
		{
			...
			debugInfo2(__FUNCSIG__, " MAC_Aを生成しました\n");
			if (debugFlag)
			{
		...
		void writeRandomChallenge(uint8_t RC1[8], uint8_t RC2[8])
		{
			...
			debugInfo2(__FUNCSIG__, " RCを書き込みました\n");
		}
		...
		void writeCardKey(uint8_t CK1[8], uint8_t CK2[8])
		{
			...
			debugInfo2(__FUNCSIG__, " CKを書き込みました\n");
		}
		...
		bool cardIdCheckMAC(uint8_t CK1[8], uint8_t CK2[8])
		{
			...
			{
				debugInfo2(__FUNCSIG__, " カード鍵照合に失敗\n");
				return false;
			}
			debugInfo2(__FUNCSIG__, " カード鍵照合に成功\n");
			return true;
		}
		...
		void enableSTATE()
		{
			...
			mc[12] = 0x01;
			updateBinary(ADDRESS_MC, mc);
			debugInfo2(__FUNCSIG__, " STATEのMAC付き書き込みを有効にしました\n");
		}
		...
		void setSTATE_EXT_AUTH(bool auth, uint8_t CK1[8], uint8_t CK2[8])
		{
			...
			if (auth)
			{
				debugInfo2(__FUNCSIG__, " 外部認証を開始します\n");
				state[0] = 0x01;
			}
			else{
				debugInfo2(__FUNCSIG__, " 外部認証を終了します\n");
				state[0] = 0x00;
			}
			updateBinaryWithMAC_A_Auto(ADDRESS_STATE, state, CK1, CK2);
			debugInfo2(__FUNCSIG__, " 外部認証処理に成功しました\n");
		}
		...
		void updateBinaryWithMAC_A_Auto(uint16_t adr, uint8_t dat[16], uint8_t CK1[8], uint8_t CK2[8])
		{
			...
			debugInfo2(__FUNCSIG__, " 自動MAC_A付き書き込み開始\n");
			...
			makeSessionKey(RC1, RC2, CK1, CK2, SK1, SK2);
			debugInfo2(__FUNCSIG__, " セッションキー生成完了\n");
			...
			debugInfo2(__FUNCSIG__, " 書き込みMAC_A生成完了\n");
			...
			debugInfo2(__FUNCSIG__, " MAC_A付き書き込み完了\n");
		}
